...
public class MessageService {
	...
	public void notifyChangeInApplicationStatus(final Status oldStatus, final Application application) {

		final Message message = this.create();
		message.setSubject("Application status changed from " + oldStatus + " to " + application.getStatus());
		...
		final Message copyManager = this.copy(message);
		copyManager.setRecipient(application.getTrip().getManager());
		copyManager.setFolder(this.folderService.findByFolderName(copyManager.getRecipient().getUserAccount().getId(), "Notification Box"));
		this.save(copyManager);

		final Message copyExplorer = this.copy(message);
		copyExplorer.setRecipient(application.getApplicant());
		copyExplorer.setFolder(this.folderService.findByFolderName(copyExplorer.getRecipient().getUserAccount().getId(), "Notification Box"));
		this.save(copyExplorer);
