...
public class SurvivalClassService {
	...
	public Collection<SurvivalClass> findCanEnrolByApplicantId(final int explorerId) {
		final Collection<SurvivalClass> result = this.survivalClassRepository.findCanEnrolByApplicantId(explorerId);
		final Collection<SurvivalClass> survivalClassesExplorer = this.explorerService.findOne(explorerId).getSurvivalClasses();
		result.removeAll(survivalClassesExplorer);
		return result;
	}
